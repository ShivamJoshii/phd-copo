...
class PipelineSmokeTest(unittest.TestCase):
    def test_run_pairwise_mapping_smoke(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertIn(rows[0]["semantic_source"], {"tfidf", "blended", "sbert"})
    ...
    def test_run_pairwise_mapping_with_tfidf_mode(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            tmp_path = Path(tmp)
            co_file = tmp_path / "co.json"
            po_file = tmp_path / "po.json"
            ...
            with pair_path.open() as f:
                rows = list(csv.DictReader(f))
            self.assertEqual(rows[0]["semantic_source"], "tfidf")
