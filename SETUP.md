...
| `attainment.py` | Stage 2 | Dataclasses + functions for the attainment math: `DirectCO = MA*w_ma + EA*w_ea`, `FinalCO = Direct*w_d + Indirect*w_i`, `PO = Σ(FinalCO_i * Map_ij) / Σ Map_ij`, scaling × 3, and target-achieved Y/N. Writes 4 outputs: CO summary, PO summary, target achievement, course summary JSON. |
...
`streamlit_app.py` is a single page with two tabs:
1. **Stage 1: Mapping** — uploads CO/PO, picks backend (`tfidf` / `sbert` / `bert`) + optional model override in the sidebar, runs the pipeline, shows the matrix and per-pair details.
2. **Stage 2: Attainment** — seeds CO list from the Stage 1 matrix automatically (or accepts an uploaded matrix), exposes editable MA/EA/Indirect values and weight/target inline, then runs the attainment analysis. `EA weight` and `Indirect weight` derive from their counterparts as `1 - x`.
